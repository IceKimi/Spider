...
public class AlibabaSearchPageParser {

	
	public static List<String> getItemIdListByHtml(String content)
	{
		List<String> itemidList = new ArrayList<String>();
		Matcher m = Pattern.compile("infoId([\\s\\S]*?),").matcher(content);
		while(m.find())
		{
			itemidList.add(m.group(1).replace("\\'", "").replace(":", ""));
		}
		return itemidList;
	}
	...
	public static Queue<String> getItemIdQueueByHtml(String content)
	{
		...
		while(m.find())
		{
			itemIdQueue.add(m.group(1).replace("\\'", "").replace(":", ""));
		}
		return itemIdQueue;
