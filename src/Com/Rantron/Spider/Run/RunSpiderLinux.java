...
public class RunSpiderLinux {
	...
	public static void main(String[] args) {
		...
		final	TaoBaoSpider searchPageSpider = new TaoBaoSpider();
		...
		for (final String keyword : keywords) {
			for (int i = 0; i < 100; i++) {
				final int b = i;
				final List<String> itemidlist = searchPageSpider.getSearchPageItemIdListBySearchWords(keyword, b,SpiderBase.SORTTYPE.SALEDESC);
						...
						try {
							...
							for (String itemid : itemidlist) {
								...
								if (!spiderCache.contain(itemid)) {
									...
									JSONObject jsonObj = (JSONObject) searchPageSpider.getItemDetailByItemid(itemid);
									baoItemDetailBase2DB.add2DB(jsonObj, keyword);
									List<String> itemComments = null;
									if (jsonObj.getString("ShopType").equals("C")) {
										itemComments = (List<String>) searchPageSpider.getItemComments(itemid, "", 1,
												SpiderBase.ECOMMERCE.TAOBAO);
										if (itemComments.size() > 0)
											itemComments2DB.add2DB(itemComments, Long.parseLong(itemid), keyword, 1, "C",
													1);
										itemComments = (List<String>) searchPageSpider.getItemComments(itemid, "", 0,
												SpiderBase.ECOMMERCE.TAOBAO);
										if (itemComments.size() > 0)
										...
										itemComments = (List<String>) searchPageSpider.getItemComments(itemid,
												jsonObj.getString("SellerId"), 1, SpiderBase.ECOMMERCE.TMALL);
										if (itemComments.size() > 0)
